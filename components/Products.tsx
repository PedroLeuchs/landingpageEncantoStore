    ...
    const ctx = gsap.context(() => {
      ...
      cards.forEach((card) => {
        ...
        const slides = Array.from(
          card.querySelectorAll<HTMLElement>(".prod-slide")
        );
        ...
        let idx = 0;
        const slideWidth = () =>
          slider?.clientWidth || slides[0]?.clientWidth || 0;
        const goTo = (i: number) => {
    ...
    <section id="products" ref={ref} className="py-28 min-h-[100vh]">
      <div className="mx-auto max-w-7xl px-4">
        ...
        <p className="mt-2 text-sm opacity-80">
          Populado por JSON, com carrossel por produto.
        </p>
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {productsInfo.map((p, i) => (
            <article
              key={i}
              className="prod-card rounded-2xl border border-black/10 p-4 bg-primary  h-74"
            >
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-background">{p.name}</h3>
              ...
              <div className="mt-3 relative">
                <div className="prod-slider overflow-hidden rounded-xl">
                  <div
                    className="prod-track flex"
                    style={{ width: `${(p.images?.length || 1) * 100}%` }}
                  >
                    {(p.images || []).map((src, idx) => (
                      <div
                        key={idx}
                        className="prod-slide shrink-0"
                        style={{ width: `${100 / (p.images?.length || 1)}%` }}
                      >
                        <img
                          src={src}
              ...
              <div className="absolute inset-x-32 bottom-2 flex items-center justify-between px-2">
                <button
                  type="button"
                  className="prod-prev hover:cursor-pointer rounded-full bg-black/60 text-white px-3 py-1 text-xs"
                >
                  ◀
                </button>
                <div className="prod-count rounded-full bg-black/60 text-white px-3 py-1 text-xs">
                  1 / {p.images.length}
                </div>
                <button
                  type="button"
                  className="prod-next hover:cursor-pointer rounded-full bg-black/60 text-white px-3 py-1 text-xs"
                >
                  ▶
                </button>
              </div>
